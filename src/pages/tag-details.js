...
class TagDetails extends Component {
  ...
  fetchTagQuotes(tag_id) {
    APIHelper.fetchTagQuotes(tag_id).then(quotes => {
      this.props.loadLearningQuotes({quotes});
    });
  }

  fetchTagBooks(tag_id) {
    APIHelper.fetchTagBooks(tag_id).then(books => {
      this.props.loadLearningBooks({books});
    });
  }
...
function mapDispatchToProps(dispatch) {
  return {
    ...
    loadLearningQuotes: quotes => dispatch(loadLearningQuotes(quotes)),
    loadLearningBooks: books => dispatch(loadLearningBooks(books))
  }
}
